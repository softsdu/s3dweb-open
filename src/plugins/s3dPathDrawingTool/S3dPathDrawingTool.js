...
import {cmnPcr, msgBox, s3dUiStatus} from "../../commonjs/common/common.js"
...
let S3dPathDrawingTool = function (){
	...
	this.calcSegmentPoints = function (fromPointInfo, toPointInfo){
		let points = [];
		points.push( new THREE.Vector3(fromPointInfo.x, thatS3dPathDrawingTool.objectZero, fromPointInfo.y));
		points.push( new THREE.Vector3(toPointInfo.x, thatS3dPathDrawingTool.objectZero, toPointInfo.y));
		return points;
	}
	...
	this.calcArcPoint3Ds = function (fromPointInfo, toPointInfo, centerPointInfo){
		if(fromPointInfo == null || toPointInfo == null){
			...
			for (let i = 0; i < points.length; i++) {
				let point = points[i];
				let point3D = new THREE.Vector3(point.x, thatS3dPathDrawingTool.objectZero, point.y);
	...
	this.calcArcRadiusPoints = function (centerPointInfo, arcCenterPoint){
		let points = [];
		points.push(new THREE.Vector3(centerPointInfo.x, thatS3dPathDrawingTool.objectZero, centerPointInfo.y));
		points.push(new THREE.Vector3(arcCenterPoint.x, thatS3dPathDrawingTool.objectZero, arcCenterPoint.y));
		return points;
	}
	...
	this.pointMapToStr = function (pathInfo){
		...
		for(let pointName in pathInfo.pointMap){
			...
			let str = pointName
				+ ","
				+ cmnPcr.decimalToStr(common3DFunction.s2v(pointInfo.x, thatS3dPathDrawingTool.manager.viewer.distanceRatio), false, thatS3dPathDrawingTool.positionFixNum)
				+ ","
				+ cmnPcr.decimalToStr(common3DFunction.s2v(pointInfo.y, thatS3dPathDrawingTool.manager.viewer.distanceRatio), false, thatS3dPathDrawingTool.positionFixNum);
			strs.push(str);
		}
			...
			let str = constraintName
				...
				+ (constraintInfo.value == null ? "" : (
					constraintInfo.constraintType === "distance"
						? cmnPcr.decimalToStr(common3DFunction.s2v(constraintInfo.value, thatS3dPathDrawingTool.manager.viewer.distanceRatio),false, thatS3dPathDrawingTool.fixNum)
	...
	this.pathInfoToStr = function (pathInfo){
		...
		for(let i = 0; i < loopedPointInfos.length; i++){
			...
			let pointStr = cmnPcr.decimalToStr(common3DFunction.s2v(pointInfo.x, thatS3dPathDrawingTool.manager.viewer.distanceRatio),false, thatS3dPathDrawingTool.fixNum)
			+ ","
			+ cmnPcr.decimalToStr(common3DFunction.s2v(pointInfo.y, thatS3dPathDrawingTool.manager.viewer.distanceRatio),false, thatS3dPathDrawingTool.fixNum)
			+ ","
