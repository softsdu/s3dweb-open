...
let S3dBuildingDWW = function (){
	...
	this.getAllWalls = function(){
		return thatBuildingDWW.getTypeObject3Ds(thatBuildingDWW.wallCategoryCodes);
	}

	this.getAllWindows = function(){
		return thatBuildingDWW.getTypeObject3Ds(thatBuildingDWW.windowCategoryCodes);
	}

	this.getAllDoors = function(){
		return thatBuildingDWW.getTypeObject3Ds(thatBuildingDWW.doorCategoryCodes);
	}
	
	this.getTypeObject3Ds = function(categoryCodes){
		let object3Ds = [];
		for(let id in thatBuildingDWW.manager.viewer.allObject3DMap){
			let object3D = thatBuildingDWW.manager.viewer.allObject3DMap[id]; 
			if(object3D.userData.info.componentInfo != null){
				if(thatBuildingDWW.checkIsCategoryType(object3D.userData.info.code,categoryCodes)){
	...
	this.onDrag = function(p){ 
		...
		let code = object3D.userData.info.code;
	...
	this.reCalcAllWallHoles = function(p){
		...
		for(let i = 0; i < wallIds.length; i++){
			...
			let oldHoleValue = wallObject3D.userData.info.parameters[thatBuildingDWW.wallHolePropertyName] == undefined ? "" : wallObject3D.userData.info.parameters[thatBuildingDWW.wallHolePropertyName];
			let needProcessHole = wallObject3D.userData.info.parameters[thatBuildingDWW.wallProcessHolePropertyName] == undefined ? true : wallObject3D.userData.info.parameters[thatBuildingDWW.wallProcessHolePropertyName];
			if(needProcessHole){
				let allHoleStrs = [];			
				for(let id in thatBuildingDWW.manager.viewer.allObject3DMap){
					let object3D = thatBuildingDWW.manager.viewer.allObject3DMap[id];
					if(object3D.userData.info.code != null){
						let code = object3D.userData.info.code;
						let isWindowOrDoor = thatBuildingDWW.checkIsWindow(code) || thatBuildingDWW.checkIsDoor(code);
						if(isWindowOrDoor){ 					
							//判断高度是否重合
							let wallBottom = common3DFunction.v2s(wallObject3D.userData.info.parameters[thatBuildingDWW.wallBottomPropertyName], thatBuildingDWW.manager.viewer.distanceRatio);
							let wallTop = wallBottom + common3DFunction.v2s(wallObject3D.userData.info.parameters[thatBuildingDWW.wallHeightPropertyName], thatBuildingDWW.manager.viewer.distanceRatio);
				...
				if(oldHoleValue != newHoleValue){
					wallObject3D.userData.info.parameters[thatBuildingDWW.wallHolePropertyName] = newHoleValue;
	...
	this.getHoleStr = function(object3D, wallId, lines){
		...
		for(let i = 0; i < lines.length; i++){
			...
			if(minDistance < thatBuildingDWW.attachDistance){
				let holeWidth = object3D.userData.info.parameters[thatBuildingDWW.holeWidthPropertyName];
				let holeHeight = object3D.userData.info.parameters[thatBuildingDWW.holeHeightPropertyName];
				let rotationAngle = common2DLine.getAngle(minLine.to, minLine.from);
				let holeStr = common3DFunction.s2v(object3D.position.x, thatBuildingDWW.manager.viewer.distanceRatio) + ","
					+ common3DFunction.s2v(object3D.position.y, thatBuildingDWW.manager.viewer.distanceRatio) + ","
					+ common3DFunction.s2v(object3D.position.z, thatBuildingDWW.manager.viewer.distanceRatio) + ","
					+ holeWidth + "," + 
					+ holeHeight + ","  
	...
	this.reGetAllWallLines = function(exceptIdMap){
		...
		for(let i = 0; i < allWallObject3Ds.length; i++){
			...
			let object3DId = wallObject3D.userData.info.id;
			if(exceptIdMap == null || exceptIdMap[object3DId] == null){
				let info = wallObject3D.userData.info;
				
				//闭合曲线墙
				if(thatBuildingDWW.checkIsCategoryType(info.code, thatBuildingDWW.polygonWallCodes)){
					let wallPath = info.parameters[thatBuildingDWW.wallPathPropertyName];
				...
				//直线墙
				if(thatBuildingDWW.checkIsCategoryType(info.code, thatBuildingDWW.directWallCodes)){
					let startPointStr = info.parameters[thatBuildingDWW.wallStartPropertyName];
					let endPointStr = info.parameters[thatBuildingDWW.wallEndPropertyName];
	...
	this.getDirectWallLines = function(startPointStr, endPointStr){
		...
		if(startPointStr != null && startPointStr.trim().length > 0 && endPointStr != null && endPointStr.trim().length > 0){ 
			...
			let from = {
				x: common3DFunction.v2s(cmnPcr.strToDecimal(startPointPartStrs[0].trim()), thatBuildingDWW.manager.viewer.distanceRatio),
				y:common3DFunction.v2s(cmnPcr.strToDecimal(startPointPartStrs[1].trim()), thatBuildingDWW.manager.viewer.distanceRatio)
			}; 
			let to = {
				x: common3DFunction.v2s(cmnPcr.strToDecimal(endPointPartStrs[0].trim()), thatBuildingDWW.manager.viewer.distanceRatio),
				y:common3DFunction.v2s(cmnPcr.strToDecimal(endPointPartStrs[1].trim()), thatBuildingDWW.manager.viewer.distanceRatio)
			}; 
			//线段墙
			lines.push({
				from: from,
	...
	this.getPolygonWallLines = function(wallPath){
		...
		if(wallPath != null && wallPath.trim().length > 0){
			...
			for(let j = 0; j < wallPointStrs.length; j++){
				...
				let x = common3DFunction.v2s(cmnPcr.strToDecimal(pStrs[0].trim()), thatBuildingDWW.manager.viewer.distanceRatio);
				let y = common3DFunction.v2s(cmnPcr.strToDecimal(pStrs[1].trim()), thatBuildingDWW.manager.viewer.distanceRatio);
				wallPoints.push({
					x: x,
					y: y
				});
